...
public class AutomaticGunScriptLPFP : MonoBehaviour {
	...
	private void Update () {
		...
		{
			if(Physics.Raycast(transform.position, transform.forward, 100.0f, LayerMask.GetMask("Monster")))
            {
				Hit();
            }
			//Shoot automatic
	...
	void Hit()
    {
		if(Physics.Raycast(gunCamera.transform.position, gunCamera.transform.forward, out HitInfo, LayerMask.GetMask("Monster")))
        {
			GameObject Clone = Instantiate(HitEffectPrefeb, HitInfo.point, Quaternion.LookRotation(HitInfo.normal));
			Destroy(Clone, 2f);
        }
    }
